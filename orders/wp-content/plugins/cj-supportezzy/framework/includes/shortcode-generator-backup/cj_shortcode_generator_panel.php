...
*/
	...
	$cj_products_text_domains = @array_keys(cjsupport_list_submenu_admin_pages('cj-products'));
	if(!empty($shortcode_tags)){
		foreach ($shortcode_tags as $key => $value) {
			$tag = explode('_', $key);
			if(@in_array($tag[0], $cj_products_text_domains)){
				$select_shortcodes[$tag[0]][] = $value;
				$shortcode_options[$value] = @unserialize(cjsupport_do_shortcode( '['.$value.' return="options"]' ));
			}
		}
	}
?>
<div class="cj-shortcode-generator-lightbox">
	...
	<div class="cj-shortcode-generator">
		<h3 class="heading">
			<img src="<?php echo cjsupport_item_path('includes_url').'/shortcode-generator/cj-icon.png'; ?>" />
			<?php _e('CSSJockey Shortcode Generator', 'cjsupport') ?>
			...
			<div class="cj-shortcode-display-container">
				<div style="padding:10px;">
					<select id="cj-select-product">
						<option value="0"><?php _e('Select Product', 'cjsupport') ?></option>
						<?php
							$cj_products = cjsupport_list_submenu_admin_pages('cj-products');
					...
					<?php
						if(!empty($select_shortcodes)){
							foreach ($select_shortcodes as $skey => $svalue) {
								...
								echo '<option value="0">'.__('Select Shortcode', 'cjsupport').'</option>';
		...
		<div id="cj-shortcode-panel" class="clearfix">
			<div id="cj-shortcode-options"><div class="shortcode-option-field"><?php _e('Select product and shortcode.', 'cjsupport') ?></div></div>
			<?php
				if (!empty($shortcode_options)) {
					...
					foreach ($shortcode_options as $key => $shortcode_options_value) {
						...
						$default_content = (isset($shortcode_options_value['default_content'])) ? $shortcode_options_value['default_content'] : __('Content goes here..', 'cjsupport');
						...
						echo cjsupport_shortcode_form($shortcode_options_value);
						echo '</form>';
						echo '</div>';
					}
				}
		...
		<?php else: ?>
			<p style="margin:10px;"><?php _e('No shortcodes available.', 'cjsupport') ?></p>
		<?php endif; ?>
