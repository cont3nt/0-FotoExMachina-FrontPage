...
global $wpdb;
require_once(sprintf('%s/helpers/init.php', cjsupport_item_path('includes_dir')));
function cjsupport_framework_info($var = null){
...
function cjsupport_item_info($var = null){
	global $cjsupport_item_vars;
	if(is_null($var)){
		return $cjsupport_item_vars['item_info'];
	}else{
		return $cjsupport_item_vars['item_info'][$var];
	}
}

function cjsupport_item_vars($var){
	global $cjsupport_item_vars;
	if(isset($cjsupport_item_vars[$var])){
		return $cjsupport_item_vars[$var];
	}
}


function cjsupport_item_options(){
	global $cjsupport_item_options;
	$option_files = cjsupport_item_vars('option_files');
	if(!empty($option_files)){
		foreach ($option_files as $key => $file) {
			if(!is_array($file)){
				require_once(sprintf('%s/'.$file.'.php', cjsupport_item_path('options_dir')));
			}
		}
	}
	return $cjsupport_item_options;
}

function cjsupport_locale_setup(){
	...
	$oname = sha1($string);
	if(get_option( $oname ) == ''){
		update_option( $oname, cjsupport_item_info('text_domain') );
	}

	// Update slugs
	$cjsupport_slugs = get_option('cjslugs');
	$cjsupport_slug = cjsupport_item_info('page_slug');
	if(!is_array($cjsupport_slugs)){
		update_option('cjslugs', array('cjsupport' => 'cjsupport'));
	}
	if(is_array($cjsupport_slugs) && !in_array('cjsupport', $cjsupport_slugs)){
		$new_array = array_merge($cjsupport_slugs, array('cjsupport' => 'cjsupport'));
		update_option('cjslugs', $new_array);
	}
}
cjsupport_locale_setup();

# Register actions and hooks
####################################################################################################

function cjsupport_admin_init(){
	// Create admin menu and page.
	add_action( 'admin_menu' , 'cjsupport_admin_menu_page');
	// Enable admin scripts and styles
	add_action( 'admin_enqueue_scripts' , 'cjsupport_admin_scripts');
	add_action( 'wp_enqueue_scripts' , 'cjsupport_frontend_scripts');
}
add_action('init', 'cjsupport_admin_init');


# Setup admin page and menu
####################################################################################################
function cjsupport_admin_menu_page(){
		global $menu;
		...
		if(!$main_menu_exists){
			$menu_icon = cjsupport_item_path('admin_assets_url', 'img/menu-icon.png');
			add_menu_page( 'CSSJockey', 'CSSJockey', 'manage_options', 'cj-products', 'cjsupport_cj_products', $menu_icon);
		}
		$menu_icon = cjsupport_item_path('admin_assets_url', 'img/menu-icon.png');
	    add_submenu_page( 'cj-products', cjsupport_item_info('page_title'), cjsupport_item_info('menu_title'), 'manage_options', cjsupport_item_info('page_slug'), 'cjsupport_admin_page_setup');
	    do_action('cjsupport_admin_menu_hook');
	    //remove_submenu_page( 'cj-products', 'cj-products' );
}

function cjsupport_cj_products(){
	require_once(sprintf('%s/cj-products.php', cjsupport_item_path('includes_dir')));
}

function cjsupport_admin_page_setup(){
    require_once(sprintf('%s/admin_page.php', cjsupport_item_path('includes_dir')));
}

# Get Admin Menu by Parent Slug
####################################################################################################
function cjsupport_list_submenu_admin_pages($parent, $offset = 3){
    global $submenu;
...
####################################################################################################
function cjsupport_admin_scripts(){

	$wp_version = get_bloginfo('version');
	$item_version = cjsupport_item_info('item_version');

	$include_pages['cjsupport'] = cjsupport_item_info('page_slug');
	$include_admin_scripts = cjsupport_item_vars('include_admin_scripts');
	...
	if(is_admin() && isset($_GET['page']) && in_array($_GET['page'], $include_pages)){
		...
		// Animate css
		// wp_enqueue_style('cj-animate-css', cjsupport_item_path('helpers_url') . '/animate.css');

		// Icons
		wp_enqueue_style('cj-fontawesome-css', cjsupport_item_path('helpers_url') . '/icons/font-awesome/css/font-awesome.min.css', null, $item_version);

		// Bootstrap
		wp_enqueue_script('cj-admin-bootstrap-js', cjsupport_item_path('helpers_url') . '/bootstrap/admin/js/bootstrap.min.js', '', $item_version, true);
		wp_enqueue_style('cj-admin-bootstrap-css', cjsupport_item_path('helpers_url') . '/bootstrap/admin/css/bootstrap.min.css', '', $item_version);

		// Quick search
		wp_enqueue_script('cj-quicksearch-js', cjsupport_item_path('helpers_url') . '/quicksearch/jquery.quicksearch.js', array('jquery'), $item_version, true);

		// jQuery UI
		wp_enqueue_script('cj-jquery_ui_timepicker_js', cjsupport_item_path('helpers_url') .'/jquery-ui/js/jquery-ui-timepicker-addon.js', array('jquery'), $item_version, true);
		wp_enqueue_style('cj-jquery_ui_css', cjsupport_item_path('helpers_url') .'/jquery-ui/css/smoothness/jquery-ui.min.css', '', $item_version);


		// Code mirror
		wp_enqueue_script('cj-codemirror_js', cjsupport_item_path('helpers_url') .'/codemirror/lib/codemirror.js', '', $item_version, true);
		wp_enqueue_script('cj-codemirror_colors_js', cjsupport_item_path('helpers_url') .'/codemirror/mode/css/css.js', '', $item_version, true);
		wp_enqueue_style('cj-codemirror_css', cjsupport_item_path('helpers_url') .'/codemirror/lib/codemirror.css', null, $item_version);
		wp_enqueue_style('cj-codemirror_theme_css', cjsupport_item_path('helpers_url') .'/codemirror/theme/ambiance.css', null, $item_version);

		// Chosen
		wp_enqueue_script('cj-chosen_js', cjsupport_item_path('helpers_url') .'/chosen/chosen.jquery.min.js', array('jquery'), $item_version, true);
		wp_enqueue_style('cj-chosen_css', cjsupport_item_path('helpers_url') .'/chosen/chosen.css', '', $item_version);

		// Admin scripts
		wp_enqueue_script('cj-admin-js', cjsupport_item_path('admin_assets_url') . '/js/admin.js', array('jquery', 'jquery-ui-core', 'jquery-ui-datepicker'), $item_version, true);


		if($wp_version >= 3.8){
			wp_enqueue_style('cj-admin-css', cjsupport_item_path('admin_assets_url') . '/css/admin-3.8.min.css', null, $item_version);
		}else{
			wp_enqueue_style('cj-admin-css', cjsupport_item_path('admin_assets_url') . '/css/admin.css', null, $item_version);
		}

	}

	wp_enqueue_style('cj-fontawesome-css', cjsupport_item_path('helpers_url') . '/icons/font-awesome/css/font-awesome.min.css', '', $item_version);
	wp_enqueue_style('cj-admin-bootstrap-css', cjsupport_item_path('helpers_url') . '/bootstrap/admin/css/bootstrap.min.css', '', $item_version);
	...
	// Color
	wp_enqueue_script('cj-color_js', cjsupport_item_path('helpers_url') .'/color/spectrum.js', '', $item_version, true);
	wp_enqueue_style('cj-color_css', cjsupport_item_path('helpers_url') .'/color/spectrum.css', '', $item_version);


	// Chosen
	wp_enqueue_script('cj-chosen_js', cjsupport_item_path('helpers_url') .'/chosen/chosen.jquery.min.js', array('jquery'),'',true);
	wp_enqueue_style('cj-chosen_css', cjsupport_item_path('helpers_url') .'/chosen/chosen.css', '', $item_version);

	wp_enqueue_script('cj-global-admin-js', cjsupport_item_path('admin_assets_url') .'/js/global-admin.js', array('jquery', 'media-upload'), '', $item_version, true);
	wp_enqueue_style('cj-global-admin-css', cjsupport_item_path('admin_assets_url') .'/css/global-admin.css', '', $item_version);

	wp_enqueue_style('cj-shortcode-generator-css', cjsupport_item_path('admin_assets_url') .'/css/shortcode-generator.css', '', $item_version);

}

# Setup admin scripts and styles
####################################################################################################
function cjsupport_frontend_scripts(){
	$wp_version = get_bloginfo('version');
	$item_version = cjsupport_item_info('item_version');
	if(!is_admin()){
		wp_register_style( 'cj-frontend-css', cjsupport_item_path('framework_url').'/assets/frontend/css/cj-global.css', null, $item_version, 'screen' );
...
####################################################################################################
function cjsupport_item_path($var = null, $path = null, $return = null){
	if(cjsupport_item_info('item_type') == 'plugin'){
	...
	}elseif(cjsupport_item_info('item_type') == 'theme'){
...
####################################################################################################
function cjsupport_item_upgrades(){
	if(cjsupport_item_info('item_type') == 'theme'){
		require_once(sprintf('%s/check-updates/themes/update.php', cjsupport_item_path('includes_dir')));
	}else{
		require_once(sprintf('%s/check-updates/plugins/update.php', cjsupport_item_path('includes_dir')));
	}
}

# Render admin form for options page
####################################################################################################
function cjsupport_admin_form($options){
	require_once(sprintf('%s/admin_form.php', cjsupport_item_path('includes_dir')));
}

# Render admin form for pages other than options page
####################################################################################################
function cjsupport_admin_form_raw($options, $search_box = null, $return = null, $chzn_class = 'chzn-select-no-results', $form_submit = null){
	global $display;
	$display = '';
	require(sprintf('%s/admin_form_raw.php', cjsupport_item_path('includes_dir')));
	if(is_null($return)){
...
####################################################################################################
function cjsupport_display_form($options){
	include(sprintf('%s/frontend_forms.php', cjsupport_item_path('includes_dir')));
	return implode("\n", $display);
}

# Render shortcode options forms
####################################################################################################
function cjsupport_shortcode_form($options){
	include(sprintf('%s/shortcode_form.php', cjsupport_item_path('includes_dir')));
	return implode("\n", $display);
}

# Show Messages
####################################################################################################
function cjsupport_show_message($type = 'warning', $message){
	return '<div class="cjsupport cj-alert alert alert-'.$type.'"><div class="cj-alert-content">'.$message.'</div></div>';
}


# Returns Post default or default value for admin_form_raw
####################################################################################################
function cjsupport_post_default($field, $default = null, $array_key = null){
...
####################################################################################################
function cjsupport_get_option($var, $default = null){
	global $wpdb;
	$return = '';
	$table = cjsupport_item_info('options_table');
...
####################################################################################################
function cjsupport_get_all_options(){
	global $wpdb;
	$return = '';
	$table = cjsupport_item_info('options_table');
...
#############################################################################
function cjsupport_generate_url($option_id = null, $url_string = null, $params = array()){
	if(is_null($option_id)){
		$display[] = __('Opiton ID not defined, please check code.', 'cjsupport');
	}else{

		if(is_null($url_string)){
			$display[] = get_permalink( cjsupport_get_option($option_id) );
		}else{
			if(!empty($params)){
				$url_string = '';
				$count = 0;
			...
			$display[] = cjsupport_string(get_permalink( cjsupport_get_option($option_id) )).$url_string;
		}
	}
	return implode("\n", $display);
}
...
####################################################################################################
function cjsupport_update_option($option_name, $option_value){
	global $wpdb;
	...
	if(!is_null($option_info)){
		cjsupport_update($options_table, $update_option_data, 'option_id', $option_info->option_id);
	}
}

# Load Modules
####################################################################################################
function cjsupport_load_modules(){
	global $cjsupport_item_vars;
	$modules = $cjsupport_item_vars['modules'];
	if(!empty($modules)){
		foreach ($modules as $key => $module) {
			require_once(sprintf('%s/'.$module.'.php', cjsupport_item_path('modules_dir')));
		}
		return true;
	}else{
		return false;
...
####################################################################################################
function cjsupport_callback_url($callback  = null){
	$text_domain = cjsupport_item_info('text_domain');
	$callback = (isset($_GET['callback']) && is_null($callback)) ? $_GET['callback'] : $callback;
	if(!is_null($callback)){
		return admin_url('admin.php?page=').cjsupport_item_info('page_slug').'&callback='.$callback;
	}else{
		return admin_url('admin.php?page=').cjsupport_item_info('page_slug');
	}
}

// Save Post Types
####################################################################################################
function cjsupport_save_post_types(){
...
add_action('admin_footer', 'cjsupport_save_post_types');

// return saved nav menus
function cjsupport_navigation_menus(){
...
// Register post types
#############################################################################
function cjsupport_register_post_types($custom_post_type = null) {
	if($custom_post_type == null){
		global $cjsupport_item_vars;
		$custom_post_types = @$cjsupport_item_vars['custom_post_types'];
...
function cjsupport_register_taxonomies($custom_taxonomies = null) {
	global $cjsupport_item_vars;
	if(is_null($custom_taxonomies) || $custom_taxonomies == ''){
		$custom_taxonomies = @$cjsupport_item_vars['custom_taxonomies'];
	}else{
...
####################################################################################################
function cjsupport_meta_boxes(){
	if ( ! class_exists( 'cmb_Meta_Box' ) ){
		require_once(sprintf('%s/metabox/init.php', cjsupport_item_path('helpers_dir')));
	}
}
...
####################################################################################################
function cjsupport_install_plugins(){
	global $cjsupport_item_vars, $cjsupport_register_plugins;
	$cjsupport_register_plugins = $cjsupport_item_vars['install_plugins'];
	if(!empty($cjsupport_register_plugins)){
		require_once(sprintf('%s/install-plugins/register_plugins.php', cjsupport_item_path('includes_dir')));
	}
}

# Shortcode Generator
####################################################################################################
function cjsupport_shortcode_generator(){
	require_once(sprintf('%s/shortcode_generator.php', cjsupport_item_path('includes_dir')));
}

# Raw code form a shortcode
####################################################################################################
/*function cjsupport_formatter($content) {
       $new_content = '';
...
add_filter('the_content', 'cjsupport_formatter', 99);*/


# Run shortcodes via PHP
####################################################################################################
function cjsupport_do_shortcode($shortcode){
...
####################################################################################################
function cjsupport_message($type, $message, $close = null){
	$close_btn = '';
	if(!is_null($close)){
		$close_btn = '<a class="alert-close" href="#close" title=""><i class="cj-icon icon-remove"></i></a>';
	}
	return '<div class="cj-alert rounded alert-'.$type.'"><div class="cj-alert-content">'.__($message, 'cjsupport').$close_btn.'</div></div>';
}

# Rearrange Files for sorting
####################################################################################################
function cjsupport_reArrayFiles(&$file_post)
{
    $file_ary = array();
...
# Sort array by array
####################################################################################################
function cjsupport_sortArrayByArray(Array $array, Array $orderArray) {
    $ordered = array();
...
# Database Insert
####################################################################################################
function cjsupport_insert($table, $data){
	global $wpdb;
...
# Database Update
####################################################################################################
function cjsupport_update($table, $data, $id_field, $id_value)
{
	global $wpdb;
	foreach ($data as $field => $value) {
...
# Format URL string
####################################################################################################
function cjsupport_string($string){
	if(strpos($string, '?') > 0){
		return $string.'&';
	}else{
		return $string.'?';
...
####################################################################################################
function cjsupport_unique_string(){
...
####################################################################################################
function cjsupport_google_fonts_string(){
	global $cjsupport_item_vars;
	$google_fonts = cjsupport_get_option( 'google_fonts' );
	$load_google_fonts_array = array();
	if(!empty($google_fonts)):
		$google_fonts_keys = array_keys(cjsupport_get_option( 'google_fonts' ));
		$item_options = cjsupport_item_options();
		foreach ($item_options as $key => $options) {
			foreach ($options as $key => $option) {
				if($option['type'] == 'font'){
					$font_vars = cjsupport_get_option($option['id']);
...
####################################################################################################
function cjsupport_email($emaildata, $content_above = null, $content_below = null, $attachments = null, $service = 'wp-mail'){
	$to = $emaildata['to'];
	...
	if($service == 'wp-mail'){
		...
		add_filter( 'wp_mail_content_type', 'cjsupport_set_html_content_type_for_email');
		wp_mail($to, $subject, $message, $headers, $attachments);
		remove_filter( 'wp_mail_content_type', 'cjsupport_set_html_content_type_for_email');
	}else{
		$headers = "From: {$from_name} <{$from}>" . "\r\n\\";
		$headers .= "Reply-To: {$from}\r\n";
		$headers .= "Return-Path: {$from}\r\n";
...
function cjsupport_set_html_content_type_for_email(){
	return 'text/html';
}

function cjsupport_debug_email($to, $subject, $array){
	$message = '';
	if(is_array($array)){
...
####################################################################################################
function cjsupport_is_email_valid($email){
	return preg_match("/^[_a-z0-9-]+(\.[_a-z0-9+-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,})$/i", $email);
}

# Check if password string is stong enough
####################################################################################################
function cjsupport_is_password_strong($pass_string, $compare = 1) {
   $r1='/[A-Z]/';  //Uppercase
   $r2='/[a-z]/';  //lowercase
...
####################################################################################################
function cjsupport_is_json($string) {
	json_decode($string);
...
####################################################################################################
function cjsupport_user_info($input, $var = null){
	global $wpdb;
	...
	if(!empty($user)){
		...
		foreach ($usermeta as $key => $value) {
			if(isset($value->meta_key) && !in_array($value->meta_key, $user_data_keys)){
		...
		if(!is_null($var)){
			...
			$return_user_data['user_role'] = cjsupport_user_role($return_user_data['ID']);
			return $return_user_data;
		}
...
####################################################################################################
function cjsupport_parse_usermeta($user_id_or_email, $string = '%%display_name%%'){
	$user_info = cjsupport_user_info($user_id_or_email);
...
####################################################################################################
function cjsupport_user_role($user_info_or_id){
	global $wpdb, $wp_roles;
	if(is_array($user_info_or_id)){
		$uid = cjsupport_user_info($user_info_or_id, 'ID');
	}else{
		$uid = $user_info_or_id;
	}
...
####################################################################################################
function cjsupport_create_unique_username($user_login_string, $separator = '', $first = 1){
    ...
    if(!username_exists($user_login)){
        ...
        if(!username_exists( $new_user_login )){
        	...
        	return cjsupport_create_unique_username($new_user_login, $separator = '', $first = 1);
        }
    }
}
...
####################################################################################################
function cjsupport_create_unique_record($string, $db_table, $column){
	global $wpdb;
	...
	if(is_null($check_existing)){
        ...
        if(is_null($check_existing)){
        	...
        	return cjsupport_create_unique_record($string, $db_table, $column);
        }
	}
}
...
####################################################################################################
function cjsupport_set_cookie($name, $value='', $expire = 86400, $path='/', $domain='', $secure=false, $httponly=false){
    $_COOKIE[$name] = $value;
    $expire = time() + $expire;
...
# Get cookie value
####################################################################################################
function cjsupport_get_cookie($name){
...
####################################################################################################
function cjsupport_local_date($format, $date = null){
	if($date == null){
...
####################################################################################################
function cjsupport_wp_date($timestamp, $time = null){
...
####################################################################################################
function cjsupport_set_timezone(){
	$timezone_string = get_option('timezone_string');
...
function cjsupport_get_timezone(){
	return date_default_timezone_get();
}

# Returns nice time string
####################################################################################################
function cjsupport_time_ago($ptime){
    $etime = time() - $ptime;
    if ($etime < 1){
        return __('Just now', 'cjsupport');
    }
    $a = array(
    	12 * 30 * 24 * 60 * 60  =>  'year',
		30 * 24 * 60 * 60       =>  'month',
    ...
    $singular = array(
    	'year' => __('year', 'cjsupport'),
		'month' => __('month', 'cjsupport'),
		'day' => __('day', 'cjsupport'),
		'hour' => __('hour', 'cjsupport'),
		'minute' => __('minute', 'cjsupport'),
		'second' => __('second', 'cjsupport'),
    );
    $plurals = array(
    	'year' => __('years', 'cjsupport'),
		'month' => __('months', 'cjsupport'),
		'day' => __('days', 'cjsupport'),
		'hour' => __('hours', 'cjsupport'),
		'minute' => __('minutes', 'cjsupport'),
		'second' => __('seconds', 'cjsupport'),
    );
    foreach ($a as $secs => $str){
        $d = $etime / $secs;
        if ($d >= 1){
            $r = round($d);
            return $r . ' ' . ($r > 1 ? $plurals[$str] : $singular[$str]) .' '. __('ago', 'cjsupport');
        }
    }
}
...
####################################################################################################
function cjsupport_trim_text($str, $cut = 200, $after_trim = ''){
...
####################################################################################################
function cjsupport_current_url($only_url = null, $port = null){
...
####################################################################################################
function cjsupport_parse_domain($url, $return = 'host'){
...
####################################################################################################
function cjsupport_remove_querystring_var($url, $key) {
...
####################################################################################################
function cjsupport_currency($symbol = '$', $amount = null, $align = 'left'){
...
####################################################################################################
function cjsupport_current_ip_address(){
	return $_SERVER['REMOTE_ADDR'];
}

# Check if its localhost or webhost
####################################################################################################
function cjsupport_is_local(){
...
# Check if its localhost or webhost
####################################################################################################
function cjsupport_get_exchange_rate($from = 'USD', $to = 'INR'){
...
# File Uplaod
####################################################################################################
function cjsupport_file_upload($field_name, $allowed_width = null, $allowed_height = null, $allowed_file_types = null, $output = 'guid', $allowed_file_size = null){
	global $wpdb;
	...
	if(!is_null($allowed_file_size) && $file_size > ($allowed_file_size * $KB)){
		$errors[] = sprintf(__('File size must be below %s kilobytes.', 'cjsupport'), $allowed_file_size);
	}

	list($img_width, $img_height) = @getimagesize($tempFile);

	if(!is_null($allowed_width) && $img_width != $allowed_width){
		$errors[] = sprintf(__('Image width must be %s pixels.', 'cjsupport'), $allowed_width);
	}

	if(!is_null($allowed_height) && $img_width != $allowed_height){
		$errors[] = sprintf(__('Image height must be %s pixels.', 'cjsupport'), $allowed_height);
	}

	if(!is_null($allowed_file_types) && !in_array(str_replace('.', '', $ext), explode('|', $allowed_file_types))){
		$errors[] = __('Invalid file type.', 'cjsupport');
	}
...
# File Uplaods
####################################################################################################
function cjsupport_file_uploads($field_name, $allowed_width = null, $allowed_height = null, $allowed_file_types = null, $output = 'guid', $allowed_file_size = null){
	global $wpdb;
...
# Template Type
####################################################################################################
function cjsupport_template_type(){
	global $wp_query;
	$return = '';
...
# Template Type Name
####################################################################################################
function cjsupport_template_type_name(){
	global $wp_query;
	$return = '';
	if(is_404()){ $return = '404'; }
	if(is_archive()){ $return = 'archive'; }
...
# Template Title
####################################################################################################
function cjsupport_template_title($post = null){
	global $wp_query, $post;

	$template = str_replace('content/', '', cjsupport_template_type());
	...
	if(is_404()){ $return = '<span class="highlight-term">'.__('Oops!! Page not found.', 'cjsupport').'</span>'; }
	if(is_archive()){ $return = sprintf(__('Archive for <span class="highlight-term">"%s"</span>', 'cjsupport'), @$archive_title ); }
	if(is_attachment()){ $return = '<span class="highlight-term">'.$post->post_title.'</span>'; }
	if(is_author()){ $return = '<span class="highlight-term">'.__('Articles posted by: ', 'cjsupport').ucwords(cjsupport_user_info($post->post_author, 'display_name')).'</span>'; }
	if(is_category()){ $return = sprintf(__('Archive for <span class="highlight-term">"%s"</span> category', 'cjsupport'), single_cat_title( '', false )); }
	if(is_front_page()){ $return = 'front_page'; }
	if(is_home()){ $return = '<span class="highlight-term">'.__('Recent Blog Posts', 'cjsupport').'</span>'; }
	if(is_page()){ $return = '<span class="highlight-term">'.$post->post_title.'</span>'; }
	if(is_search()){ $return = sprintf(__('Search results for <span class="highlight-term">"%s"</span>', 'cjsupport'), @$_GET['s']); }
	if(is_single()){ $return = '<span class="highlight-term">'.$post->post_title.'</span>'; }
	if(is_tag()){ $return = sprintf(__('Posts tagged <span class="highlight-term">"%s"</span>', 'cjsupport'), single_tag_title( '', false )); }
	if(is_tax()){ $return = sprintf(__('Archive for <span class="highlight-term">"%s"</span>', 'cjsupport'), single_term_title( '', false )); }
...
####################################################################################################
function cjsupport_featured_image($size = null, $single = false){
	global $post;
	if(has_post_thumbnail( $post->ID )){
		if(!is_array($size)){
			...
			$image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), $size );
			$image = cjsupport_resize_image($image[0], $size[0], $size[1], true, $single);
		}
		if($single){
			$return = $image[0];
		}else{
...
####################################################################################################
function cjsupport_post_featured_image($post_id, $size = null, $single = false, $default_text = 'No Thumbnail'){
	global $post;
	if(has_post_thumbnail( $post_id )){
		if(!is_array($size) && !is_null($size)){
			...
			$image = wp_get_attachment_image_src( get_post_thumbnail_id( $post_id ), $size );
			$image = cjsupport_resize_image($image[0], $size[0], null, true, false);
		}
		if($single){
			$return = $image[0];
		}else{
...
####################################################################################################
function cjsupport_resize_image($src, $width, $height = null, $crop = false, $single = false){
	require_once(sprintf('%s/aq_resizer.php', cjsupport_item_path('helpers_dir')));
...
####################################################################################################
function cjsupport_file_info($url_or_id){
	global $wpdb;
...
# Get Gravatar URL
####################################################################################################
function cjsupport_gravatar_url($user_id_or_email, $size = 150){
	$user_email = cjsupport_user_info($user_id_or_email, 'user_email');
...
# Get WP Avatar URL
####################################################################################################
function cjsupport_wp_avatar_url($user_id_or_email, $size = 150){
	$user_id = cjsupport_user_info($user_id_or_email, 'ID');
...
# Get url of an image
####################################################################################################
function cjsupport_get_image_url($image){
...
####################################################################################################
function cjsupport_post_by_metakey($meta_key, $meta_value){
	global $wpdb;
...
####################################################################################################
function cjsupport_post_info($post_id, $var = null){
	global $wpdb;
...
function cjsupport_post_meta($meta_key, $default = null, $post_info){
...
####################################################################################################
function cjsupport_post_terms($post_id, $taxonomy = null){
	global $wpdb;
	$post = get_post($post_id);
	if(!is_null($taxonomy)){
...
####################################################################################################
function cjsupport_post_count($term_id){
	global $wpdb;
...
function cjsupport_terms_array($taxonomy_slug, $object = false){
	global $wpdb;
	$group_args = array(
		'orderby' => 'name',
	    'order' => 'ASC',
...
function cjsupport_load_iconset_css(){
	$enable_fonts = @cjsupport_item_vars('load_extras');
	if(isset($enable_fonts['icomoon-icons']) && $enable_fonts['icomoon-icons'] == 1){
		$icons_url = cjsupport_item_path('framework_url').'/assets/helpers/icons/icomoon/all/';
		for ($i=1; $i < 13; $i++) {
			echo '<link rel="stylesheet" id="cjsupport_iconset-'.$i.'"  href="'.$icons_url.'ncicons-'.$i.'/style.css" type="text/css" media="all" />';
		}
	}
}
add_action('wp_head', 'cjsupport_load_iconset_css');



function cjsupport_color_brightness($hex, $steps) {
...
function cjsupport_item_assistant_holder(){
	echo '<div class="cj-assistant">';
	echo do_action('cj_assistant_hook');
	echo '</div>';
}
add_action('admin_footer', 'cjsupport_item_assistant_holder');


function cjsupport_convert_html($html){
...
add_filter( 'no_texturize_shortcodes', 'cjsupport_shortcode_no_wptexturize' );
function cjsupport_shortcode_no_wptexturize($shortcodes){
    $shortcodes[] = 'rev_slider';
    return $shortcodes;
}


function cjsupport_add_settings_link( $links ) {
    $settings_link = '<a href="'.cjsupport_callback_url('core_welcome').'">'.__('Settings', 'cjsupport').'</a>';
    $settings_link .= ' | <a href="'.cjsupport_callback_url('core_uninstall').'">'.__('Uninstall', 'cjsupport').'</a>';
  	array_push( $links, $settings_link );
  	return $links;
}

function cjsupport_get_multi_array_values($array, $return = array()) {
    for($x = 0; $x <= count($array); $x++) {
        if(isset($array[$x]) && is_array($array[$x])) {
            $return = cjsupport_get_multi_array_values($array[$x], $return);
        }
        else {
            if(isset($array[$x])) {
                $return[] = $array[$x];
...
function cjsupport_load_debug_file(){
	if(isset($_GET['cjsupport_debug']) && $_GET['cjsupport_debug'] == 1){
		require_once(sprintf('%s/debug.php', cjsupport_item_path('item_dir')));
		die();
	}
}
add_action('init', 'cjsupport_load_debug_file');

function cjsupport_countries_array($country = null){
...
function cjsupport_fontawesome_icons_array($icon = null){
...
function cjsupport_google_fonts_array($font = null){
...
###############################################################################################################
function cjsupport_geo_address_by_zipcode($zip, $only_zip = false){
	...
	if($status=="OK"){
		if(count($data['results']) > 0){
			foreach ($data['results'] as $key => $result) {
				...
				$address = cjsupport_geo_address_by_latlng($location['lat'], $location['lng']);
				if(is_array($address)){
	    	...
	    	$return = __('No locations found.', 'cjsupport');
	    }
	}else{
	    $return = __('No locations found.', 'cjsupport');
	}
...
#
###############################################################################################################
function cjsupport_geo_address_by_latlng($lat,$lng){
...
if(cjsupport_item_info('item_type') == 'plugin'){
	...
	add_filter( "plugin_action_links_$plugin", 'cjsupport_add_settings_link' );
}

if(cjsupport_item_info('item_id') != 'NA'){
	require_once(sprintf('%s/upgrades/init.php', cjsupport_item_path('includes_dir')));
}

require_once(sprintf('%s/init.php', cjsupport_item_path('includes_dir')));